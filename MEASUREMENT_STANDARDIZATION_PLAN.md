...
| 11b | Next; not started | Atomic no-replace publication: NTFS collisions/faults; explicit opt-in SMB tests; untested filesystems not claimed safe. |
| 11a review fixes | Completed | Real fsync/descriptor failures now propagate in both writers; only streams without a supported file descriptor skip syncing. Shared preflight validates required metadata, schema/version and exact saved-column units before writing or opening/truncating a path. CSV reader parses logical records so quoted newlines survive, preserves free-form metadata as text (including leading zeros, NA and empty strings), and decodes only schema version/Boolean fields. Added 25 regression cases; persistence suites: 51 passed; full suite: 1095 passed, 2 xfailed in 19.64s. Atomic/no-replace publication remains 11b work; the path convenience writer is not an atomic publisher. |
| 11c onward | Not started | Bundles, partials and recovery. |
