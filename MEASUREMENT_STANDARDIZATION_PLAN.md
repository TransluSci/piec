...
| 11a | Next; not started | Handle writer and schema metadata: One-row CSV layout and exact JSON unit round trips. |
| 9a/10a/10b review fixes | Completed | Execution claims validate tokens before changing owner/results or entering safing; rejected duplicate/stale/concurrent calls leave the active worker untouched. Runner validates options before reservation and finalizes unconsumed reservations on thread construction/start or worker preflight failure without I/O. New starts are rejected until the previous worker finishes terminal delivery, preventing false close readiness. Terminal data snapshots replace stale display windows, including empty analyzed results. Added 11 regression tests. Execution ownership commit: `794dd50`; runner recovery/close commit: `6343326`; final snapshot fix accompanies this entry. Suite excluding pending 11a persistence tests: 1044 passed, 2 xfailed; full working-tree suite including pending persistence: 1059 passed, 2 xfailed. Pending 11a files were preserved and are not approved by this review; resume by reviewing/completing 11a only. |
| 11b onward | Not started | Atomic no-replace publication, bundles, partials and recovery. |
