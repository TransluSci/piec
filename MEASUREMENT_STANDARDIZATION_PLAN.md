...
| 9b | Completed | Implemented piecewise execution sessions via `MeasurementSession` context manager (`BaseMeasurement.session`) and standalone operation scopes (`configure_instruments()`, `capture_data()`). Enforced single command owner bound to calling thread, rejection of nested sessions, rejection of `run_experiment()` inside sessions, rejection of cross-thread hardware calls, and at most one capture per session. Clean session exit safes before analysis and saves only on explicit `save=True` (default `save=False`). Configuration-only session skips analysis and saving. Standalone scopes reserve transient generations, safe on exit, emit single terminal event, and do not save. Idle `safe_shutdown()` acquires command lease and updates safety status while non-owner calls during active runs latch Stop and raise `RuntimeError("Shutdown deferred to execution owner")`. Added 26 contract tests in `tests/test_measurement_sessions.py`. Focused suite: 26 passed; full suite: 980 passed, 2 xfailed in 20.87s on Python 3.13.2. Checkpoint 9c remains next. |
| 9c | Next; not started | Fault handling hardening. |
| 10a onward | Not started | Continue engine and family slices. |
