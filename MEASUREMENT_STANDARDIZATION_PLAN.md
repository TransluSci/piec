...
| 12 | Completed | Rewrote developer guide (`src/piec/measurement/MEASUREMENT_DEVELOPER_GUIDE.md`) and documentation (`docs/source/contributing/adding_measurement.rst`) against the implemented `BaseMeasurement` engine. Documented architectural layering, BaseMeasurement subclassing and constructor rules, protected hooks vs public wrappers, guaranteed hardware safing, ShutdownAttemptRecorder, plain column names, column_units_json metadata serialization, single execution ownership, MeasurementRunner non-daemon worker and close coordination, dual DisplayQueue/ControlQueue event distribution, atomic no-replace publication, candidate reservations, multi-artifact plot bundles, partial checkpoints, and failure recovery. Extracted and validated all code blocks against python execution with 100% success. Focused suite: 490 passed, 1 skipped, 2 xfailed; full repo suite: 1147 passed, 1 skipped, 2 xfailed in 20.32s on Python 3.13.2. Checkpoint 13 is next. |
