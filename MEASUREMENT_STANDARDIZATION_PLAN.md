...
| 28f | Completed | Generic per-instance virtual hooks for AWG family (`VirtualAwg`): `waveform_hook` / `apply_hook` / `trigger_hook` injection with strict precedence over deprecated global `sample` fallback, external physics decoupling (Landau model, prep points, PUND pulse sequences kept external), flexible signature binding (`(v, t)`, `(v, t, ch)`, named params, single `v`, 0-arg notification, positional-only, `*args`/`**kwargs`), exact-once invocation without retry, declared units (`{"voltage": "V", "frequency": "Hz", "time": "s"}`), input validation before mutation across all settings, atomic configuration methods (`configure_waveform`, `configure_pulse`, `configure_trigger`), SCPI trigger dispatch (`*TRG`, `:TRIG`) and queries (`*IDN?`, `*OPC?`, `*ESR?`, `:OUTP?`, `:SOUR:FREQ?`, `:SOUR:VOLT?`), driver reset ownership preserving hooks, and instance-level sample isolation. 45 tests in `tests/test_virtual_awg_hook.py`. |
| 28g onward | Generic per-instance virtual hooks, remaining driver families one family per commit | Explicit injection overrides global fallback; no sample-specific driver branches |
