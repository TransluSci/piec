...
| 11c | Completed | Implemented multi-artifact bundle publication (`publish_artifact_bundle`), candidate filename reservation with documented grammar `{index}_{measurement_schema}.csv` and exclusive hidden markers storing full owner UUID (`reserve_candidate_filename`, `CandidateReservation`), in-progress checkpoints and terminal partial CSV writer (`write_partial_csv`), completed CSV identification (`is_completed_measurement_file`), and explicit ownership-checked age-gated cleanup (`cleanup_stale_reservations`, `cleanup_stale_partials`). Enforced publication ordering: side artifacts first, completed CSV last as completion marker. Enforced non-destructive failure rollback: removes only artifacts proven to belong to current reservation, tracks orphan cleanup failures, preserves all staging files for data recovery, never deletes foreign or pre-existing files. Added 15 tests in `tests/test_measurement_persistence_bundles.py`. Persistence suites: 62 passed, 1 skipped; all measurement suites: 499 passed, 1 skipped, 2 xfailed; full repo suite: 1131 passed, 1 skipped, 2 xfailed in 18.91s on Python 3.13.2. Checkpoint 12 remains next. |
| 12 | Next; not started | Rewrite developer guide to the implemented engine: Examples compile and show the shared API, plain columns, metadata units, and one hardware writer. |
