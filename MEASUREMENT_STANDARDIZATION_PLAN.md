...
| 27 | Completed | Implemented role-specific simulation contracts in `piec.simulation.contracts`: explicit physical units (`V`, `A`, `Ohm`, `s`, `Oe`, `deg`), frozen `LoadResponse`, plain voltage-response tuple with explicit excitation current, `DeterministicTimebase`, seeded `SimulationRole` reset protocol, `ElectricalLoadContract` with two-terminal `LoadMode.VOLTAGE_SOURCE` and `LoadMode.CURRENT_SOURCE` evaluation, bidirectional compliance limiting, `ResistorLoad`, `DiodeLoad`, `CapacitiveLoad`, and material contracts (`FieldResponsiveMaterialContract`, `AngleDependentResistanceContract`, `WaveformResponsiveMaterialContract`). Updated `HystereticMagneticMaterial`, `MagneticMaterial`, and `Ferroelectric` models to conform to contracts. Added 33 contract tests in `tests/test_simulation_contracts.py`. Full suite with Agg: **1657 passed, 1 skipped** in 62.01s on Python 3.13.2. Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 28a | Completed | Generic per-instance virtual hooks for Lock-in driver family (`VirtualLockin`): `xy_reader` / `transport_hook` injection with strict precedence over deprecated global `mag_sample` fallback, material decoupling, declared units, explicit excitation current forwarding, plain tuple voltage responses, deterministic reset preserving hooks, and instance-level `mag_sample` isolation. Added 28 contract tests in `tests/test_virtual_lockin_hook.py`. Full suite with Agg: **1725 passed, 1 skipped** in 62.31s on Python 3.13.2. Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 28b | Completed | Generic per-instance virtual hooks for DMM driver family (`VirtualDMM`): `voltage_reader` / `reader_hook` injection, precedence over global fallback, material decoupling, scalar float V response, overload preservation, non-scalar rejection, error preservation without retries, declared units, reset ownership, and instance isolation. 37 tests in `tests/test_virtual_dmm_hook.py`. Full suite with Agg: **1780 passed, 1 skipped** in 65.66s on Python 3.13.2. Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 28c | Completed | Generic per-instance virtual hooks for DC Calibrator family (`VirtualCalibrator`): `output_hook` / `field_hook` injection, precedence over global fallback, material decoupling, commanded output binding, error preservation without retries, declared units, reset ownership, and instance isolation. 46 tests in `tests/test_virtual_calibrator_hook.py`. Full suite with Agg: **1836 passed, 1 skipped** in 63.97s on Python 3.13.2. Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 28d | Completed | Generic per-instance virtual hooks for Stepper Motor family (`VirtualStepper`): `angle_hook` / `position_hook` / `step_hook` injection, precedence over global fallback, external physics decoupling, motion parameter binding, unconfirmed shutdown on failure (`moving=None`), error preservation without retries, declared units, reset ownership, and instance isolation. 72 tests in `tests/test_virtual_stepper_hook.py`. Full suite with Agg: **1920 passed, 1 skipped** in 65.09s on Python 3.13.2. Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 28e | Completed | Generic per-instance virtual hooks for Oscilloscope family (`VirtualScope`): `waveform_hook` / `channel_hook` / `data_hook` injection with strict precedence over deprecated global `sample` fallback, external physics decoupling, flexible signature binding (channel, vdiv, y_range, tdiv, x_range, points, state, kwargs, 0-arg), exact-once invocation without retry, declared units (`{"voltage": "V", "time": "s"}`), input validation before mutation, scale consistency (`y_range = 8*vdiv`, `x_range = 8*tdiv`), output normalization and validation (`DataFrame`, dict, `(voltages, times)` tuple), driver reset ownership preserving hooks, and instance-level sample isolation. Added 46 tests in `tests/test_virtual_oscilloscope_hook.py`. Full suite with Agg: **1977 passed, 1 skipped** in 62.18s on Python 3.13.2. Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 28f | Completed | Generic per-instance virtual hooks for Arbitrary Waveform Generator family (`VirtualAwg`): `waveform_hook` / `apply_hook` / `trigger_hook` injection with strict precedence over deprecated global `sample` fallback, external physics decoupling (Landau model, prep points, and PUND sequences kept external), flexible signature binding (`(v, t)`, `(v, t, ch)`, named params, single `v`, 0-arg notification, positional-only, `*args`/`**kwargs`), exact-once invocation without retry, declared units (`{"voltage": "V", "frequency": "Hz", "time": "s"}`), input validation before mutation across all settings, atomic configuration methods (`configure_waveform`, `configure_pulse`, `configure_trigger`), SCPI trigger dispatch (`*TRG`, `:TRIG`) and queries (`*IDN?`, `*OPC?`, `*ESR?`, `:OUTP?`, `:SOUR:FREQ?`, `:SOUR:VOLT?`), driver reset ownership preserving hooks, and instance-level sample isolation. Added 45 tests in `tests/test_virtual_awg_hook.py`. Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 28g | Completed | Generic per-instance virtual hooks for Sourcemeter family (`VirtualSourcemeter`): `load_hook` / `source_hook` / `measure_hook` / `transport_hook` injection with strict precedence over unhooked fallback, `ElectricalLoadContract` integration (`ResistorLoad`, `DiodeLoad`, `CapacitiveLoad`), voltage-source and current-source modes, load-solved compliance validation and `compliance_tripped` tracking, distinguishing configured setpoints from effective terminal output (0.0 V / 0.0 A when output disabled), unconfirmed state on failure (`output_on=None`), exact-once invocation without retry, declared units (`{"voltage": "V", "current": "A", "resistance": "Ohm", "time": "s"}`), input validation before mutation, driver reset ownership preserving hooks, and instance-level sample isolation. Added 73 tests in `tests/test_virtual_sourcemeter_hook.py`. Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 28h | Completed | Driver family audit & completion of Checkpoint 28: audited all remaining virtual drivers (`VirtualDaq`, `VirtualPulser`, `VirtualRFSource`) and DAQ emulator adapters (`DaqAsAwg`, `DaqAsOscilloscope`); confirmed the seven Section 11.2 generic hook roles are completed (28a–28g); DAQ/AWG/trigger tests pass (61); direct DAQ-to-scope adapter coverage is absent; no additional virtual driver hook family is required within checkpoint 28 scope. Checkpoint 28 is complete. Ready to advance to VirtualBench (Checkpoint 29). Physical checkpoints 17, 22, and 26 remain explicitly PENDING. |
| 29 | Completed | Implemented VirtualBench ownership, routing, deterministic time/noise, reset isolation and concurrent-bench tests. IV/MOKE schema integration covered without migrating existing fixtures. Next: 30a IV fixtures. See handoff for validation. |
...
protecting `VirtualInstrument._shared_mag_sample`, and driver reset ownership documented.
Checkpoint 28e completed: generic per-instance virtual hooks for the Oscilloscope family (`VirtualScope`).
Supports `waveform_hook`, `channel_hook` (alias), and `data_hook` (alias) injection with strict precedence over deprecated
global `sample` fallback, external physics decoupling (no sample- or ferroelectric-specific logic in generic driver), flexible
parameter binding (`channel`, `vdiv`, `y_range`, `y_position`, `tdiv`, `x_range`, `x_position`, `coupling`, `probe_attenuation`,
`points`, `state`, `*args`, `**kwargs`, zero-arg), exact-once invocation without error retry, declared units
(`{"voltage": "V", "time": "s"}`), input validation before mutation, 8-division scale consistency (`y_range = 8*vdiv`, `x_range = 8*tdiv`),
output normalization and validation (standardizing `pd.DataFrame`, mapping/dict, or `(voltages, times)` tuples into canonical
DataFrame with 'Time' and 'Voltage' columns, validating non-empty, matching lengths, finite non-negative times), instance isolation
protecting `VirtualInstrument._shared_fe_sample`, and driver reset ownership documented (driver reset restores default configuration
and clears acquisition points while preserving the injected hook; setup-owned models/clocks/RNG are not reset).
Checkpoint 28f completed: generic per-instance virtual hooks for the Arbitrary Waveform Generator family (`VirtualAwg`).
Supports `waveform_hook`, `apply_hook` (alias), and `trigger_hook` (alias) injection with strict precedence over deprecated
global `sample` fallback, external physics decoupling (no Landau model, prep points, or PUND logic in generic driver), flexible
signature binding (`(v, t)`, `(v, t, ch)`, named parameters `v`, `voltages`, `waveform`, `v_applied`, `data`, `t`, `times`,
`timestamps`, `time`, `channel`, `ch`, `freq`, `frequency`, `duration`, single-argument `(v)`, positional-only `(v, t, /)`,
variadic `*args`/`**kwargs`, and zero-argument notifications), exact-once invocation without error retry, declared units
(`{"voltage": "V", "frequency": "Hz", "time": "s"}`), input validation before mutation across all channels and settings
(positive finite frequencies, finite amplitudes, finite offsets, valid duty cycle / symmetry in [0.0, 100.0], positive pulse width,
non-negative pulse delay, validated polarity/slope/mode/source strings), atomic configuration methods (`configure_waveform`,
`configure_pulse`, `configure_trigger`), SCPI trigger dispatch (`*TRG`, `:TRIG`) and queries (`*IDN?`, `*OPC?`, `*ESR?`, `:OUTP?`,
`:SOUR:FREQ?`, `:SOUR:VOLT?`), instance isolation protecting `VirtualInstrument._shared_fe_sample`, and driver reset ownership
documented (driver reset restores default configuration and turns off all outputs while preserving the injected hook; setup-owned
models/clocks/RNG are not reset). Next is checkpoint 28g (or remaining virtual-driver families / next checkpoint), one family per commit.
...
| 28e | Completed | Generic per-instance virtual hooks for Oscilloscope family (`VirtualScope`): `waveform_hook` / `channel_hook` / `data_hook` injection with strict precedence over deprecated global `sample` fallback, external physics decoupling, flexible signature binding (channel, vdiv, y_range, tdiv, x_range, points, state, kwargs, 0-arg), exact-once invocation without retry, declared units (`{"voltage": "V", "time": "s"}`), input validation before mutation, scale consistency (`y_range = 8*vdiv`, `x_range = 8*tdiv`), output normalization and validation (`DataFrame`, dict, `(voltages, times)` tuple), driver reset ownership preserving hooks, and instance-level sample isolation. 46 tests in `tests/test_virtual_oscilloscope_hook.py`. |
| 28f | Completed | Generic per-instance virtual hooks for Arbitrary Waveform Generator family (`VirtualAwg`): `waveform_hook` / `apply_hook` / `trigger_hook` injection with strict precedence over deprecated global `sample` fallback, external physics decoupling, flexible signature binding (`(v, t)`, `(v, t, ch)`, named params, single `v`, 0-arg, positional-only, `*args`/`**kwargs`), exact-once invocation without retry, declared units, input validation before mutation across all settings, atomic configuration methods, SCPI trigger dispatch and queries, driver reset ownership preserving hooks, and instance-level sample isolation. 45 tests in `tests/test_virtual_awg_hook.py`. |
| 28g | Completed | Generic per-instance virtual hooks for Sourcemeter family (`VirtualSourcemeter`): `load_hook` / `source_hook` / `measure_hook` / `transport_hook` injection with strict precedence over unhooked fallback, `ElectricalLoadContract` integration (`ResistorLoad`, `DiodeLoad`, `CapacitiveLoad`), voltage-source and current-source modes, load-solved compliance validation and `compliance_tripped` tracking, distinguishing configured setpoints from effective terminal output (0.0 V / 0.0 A when output disabled), unconfirmed state on failure (`output_on=None`), exact-once invocation without retry, declared units (`{"voltage": "V", "current": "A", "resistance": "Ohm", "time": "s"}`), input validation before mutation, driver reset ownership preserving hooks, and instance-level sample isolation. 73 tests in `tests/test_virtual_sourcemeter_hook.py`. |
...
| 30c–30d | Migrate FE/AMR simulation fixtures one family per commit | Physical/virtual schemas agree; preserve applicable generic-driver fallback tests |
...
Checkpoint 28g review: corrected load-solved compliance validation, unknown-output readback, setup-clock capacitor integration, hook binding and SCPI error propagation. See MEASUREMENT_HANDOFF.md for regression results and the remaining-family audit before 28h/29. Physical 17/22/26 remain PENDING.
