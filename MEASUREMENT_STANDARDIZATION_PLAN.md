...
and clears acquisition points while preserving the injected hook; setup-owned models/clocks/RNG are not reset). Next is checkpoint 28f,
...
| 28f onward | Generic per-instance virtual hooks, remaining driver families one family per commit | Explicit injection overrides global fallback; no sample-specific driver branches |
