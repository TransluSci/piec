...
### Review corrections and next task

The checkpoint completion is scoped to Section 11.2 hooks, not removal of all shared state or proof that every virtual driver/adapter is fully isolated. VirtualInstrument still initializes and exposes shared fallback samples; retain that behavior until checkpoint 31. Bench-owned objects must use explicit injection rather than those globals.

Review verification: the three cited DAQ/AWG/trigger suites were rerun: **61 passed in 1.69s**. No production code changed in this documentation review; the full-suite numbers below are the original implementation report, not a new full run.

Gemini: implement checkpoint 29 only. Make VirtualBench own models, instruments, routing, deterministic clock/RNG and reset. Cover two concurrent benches, independent reset, both source modes and compliance, and the MOKE calibration/material/DMM routing in Section 11.3. Treat DAQ-to-scope integration as unverified if introducing that adapter. Preserve the reviewed sourcemeter behavior and all physical PENDING gates. Do not bundle measurement-fixture migration (30), fallback retirement (31), or additional AMR adapters. Run focused and full tests, update this handoff, commit separately and stop for review.


- **Status**: **Completed**. Checkpoint 28 virtual-driver hook modernisation is **COMPLETE** for the seven hook roles scoped in Section 11.2.
...
- **Comprehensive Virtual Driver Family Audit**:
  - **Audited 10 Virtual Driver Implementations and 2 Emulator Adapters**:
    ...
    8. `VirtualDaq` (`src/piec/drivers/daq/virtual_daq.py`): **Audited**. Has no direct sample-dependent acquisition/output logic; inherits the shared fallback from VirtualInstrument. Generates fixed synthetic 10 Hz sine wave for scans and returns stored scalar 0.0 V. No direct usage found in current measurement implementations.
    9. `DaqAsAwg` & `DaqAsOscilloscope` (`src/piec/drivers/emulators/`): **Audited**. Per Section 11.1 ("DAQ-to-AWG/scope adapters only if their own interface tests expose a defect"): the cited DAQ/AWG/trigger suites (`tests/test_awg_contract.py`, `tests/test_daq_contract.py`, `tests/test_daq_trigger_pulses.py`) pass 61 tests. They exercise DAQ contracts and DaqAsAwg behavior, but do not instantiate DaqAsOscilloscope. The scope-shaped dictionary in test_waveform_reader.py only tests normalization, not the adapter. Source inspection found no direct shared-sample access in either adapter; full adapter correctness is not established by these tests.
    10. `VirtualPulser` (`src/piec/drivers/pulser/virtual_pulser.py`): **Audited**. Pure parameter state tracker. Has no direct sample-dependent acquisition/output logic. No direct usage found in current measurement implementations; inherited shared fallback remains.
    11. `VirtualRFSource` (`src/piec/drivers/rf_source/virtual_rf_source.py`): **Audited**. Pure parameter state tracker. Has no direct sample-dependent acquisition/output logic. No direct usage found in current measurement implementations; inherited shared fallback remains.
  - **Conclusion & Inventory Exhaustion**:
    The seven generic hook roles in Section 11.2 have implementations and dedicated regression coverage. No additional hook family is required by this checkpoint. VirtualDaq, VirtualPulser and VirtualRFSource have no direct sample-dependent acquisition/output logic, but inherit VirtualInstrument and its shared fallback properties. No direct use was found in the current measurement implementations; this is not a claim that they cannot be passed through generic instrument interfaces. VirtualInstrument shared-state retirement remains checkpoint 31 after consumer migration. Checkpoint 28 is **COMPLETE**.
- **Validation**:
