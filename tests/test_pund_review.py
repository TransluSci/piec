...
def measurement(tmp_path, **kwargs):
    return ThreePulsePund(VirtualAwg(simulation_points=50), VirtualScope(simulation_points=50), output_dir=tmp_path, **kwargs)
...
def test_pund_dc_offset_includes_idle_and_does_not_alter_current(tmp_path):
    from piec.analysis.pund import process_pund
    m = measurement(tmp_path, offset=2.0)
...
def app(tmp_path):
    spec = importlib.util.spec_from_file_location('fe_gui_pund_review', ROOT/'Measurements/Ferroelectric Testing/FE_testing_GUI.py')
    ...
    gui.measurement_type = Mock(get=Mock(return_value='ThreePulsePund'))
    gui.x_axis = Mock(get=Mock(return_value='time (s)'))
    gui.y_axis = Mock(get=Mock(return_value='current (A)'))
    gui.ax = Mock(); gui.canvas = Mock()
    gui.dynamic_inputs = {}
    gui.update_dynamic_defaults = Mock()
    m = measurement(tmp_path, save_plots=False)
    m.awg.close = Mock(); m.osc.close = Mock()
    gui._instruments = []
    def create():
        gui.experiment = m
        gui._instruments = [m.awg, m.osc]
    gui._create_experiment = create
    return gui, mod, m

def test_gui_pund_completion_uses_memory_and_releases_instruments(tmp_path, monkeypatch):
    ...
    m.awg.close.assert_called_once()
    m.osc.close.assert_called_once()

def test_gui_pund_stop_and_deferred_close(tmp_path):
    import threading
    gui, mod, m = app(tmp_path)
    ...
    try:
        gui.on_closing()
        gui._finish_close.assert_not_called()
        m.awg.close.assert_not_called()
        gui._create_experiment = Mock()
        gui.run_measurement()
        gui._create_experiment.assert_not_called()
    finally:
        release.set()
    assert gui.runner.join(timeout=5)
    gui._poll_events()
    assert gui.runner.run_state == RunState.ABORTED
    gui._finish_close.assert_called_once()
    m.awg.close.assert_called_once()

def test_gui_pund_unsafe_retains_connections_until_retry(tmp_path, monkeypatch):
    gui, mod, m = app(tmp_path)
