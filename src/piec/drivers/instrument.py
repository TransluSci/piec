...
class Instrument(metaclass=AutoCheckMeta):
    ...
    def reset(self):
        """
        Resets the instrument to its default / safe idle state.

        Subclasses should override this method to perform hardware-specific
        reset operations (e.g., *RST for SCPI, stopping scans for DAQ, or
        zeroing positions for motion controllers). Also re-initialises
        the internal state tracker.
        """
        self._initialize_state()

    def clear(self):
        """
        Clears the instrument's status registers, error queue, or communication buffers.

        Useful for clearing any errors or stale status messages generated by
        previous operations. Subclasses should override this for device-specific
        clearing (e.g., *CLS for SCPI, serial RX/TX flush for microcontrollers).
        """
    ...
    def initialize(self):
        ...
        to a known good starting state before a measurement.
